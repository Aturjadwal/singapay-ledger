# ARCHIVED — pre-implementation design notes

> **STATUS: HISTORICAL. Do not use as a reference for how this system works.**
> Archived 2026-09-13. This was `AGENTS.md`.
>
> It is a design document written **before** the ledger was built, against the previous
> payment gateway, and later put through a global DOKU → Singapay find-and-replace. That
> rename changed the names without changing the design, so much of what follows describes a
> system that was never built:
>
> - **Tables that do not exist:** `ledgers`, `ledger_transactions`, `expected_balances`,
>   `reconciliation_logs` (dropped by migration 008).
> - **A settlement CSV.** Singapay publishes no settlement file at all. The format quoted
>   below is the previous gateway's, relabelled.
> - **"Manual reconciliation required — Singapay doesn't provide webhooks."** It does, and
>   the settlement webhook is implemented.
> - **`gateway_transaction_id` on `product_transactions`.** It was built on
>   `payment_requests` instead (migration 023), because the instrument and the payment are
>   different entities for virtual account and payment link.
> - **"Migration Files Needed" and "In Progress / TODO"** bear no relation to the migrations
>   or the code that shipped.
>
> It is kept because the reasoning about balance safety gates, FIFO matching and discrepancy
> handling records why certain paths were considered and not taken.
>
> **For how the system actually works, read:**
> [`../../README.md`](../../README.md) · [`../ENTITIES.md`](../ENTITIES.md) ·
> [`../101-payment-execution.md`](../101-payment-execution.md) ·
> [`../102-settlement-reconciliation.md`](../102-settlement-reconciliation.md) ·
> [`../104-fee-mismatch-reconciliation.md`](../104-fee-mismatch-reconciliation.md)
